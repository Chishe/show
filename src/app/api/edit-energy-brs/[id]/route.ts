...
export async function PUT(req: NextRequest,  context: { params: Promise<Record<string, string>> }
  ...
  const { id } = await context.params;
    ...
    try {
        ...
        return new NextResponse(
            JSON.stringify({
                message: 'Energy BRS status updated successfully',
                result: result.rows[0],
            }),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
    } catch (error) {
        console.error('Error updating energy BRS status:', error);
        return new NextResponse(
            JSON.stringify({ message: 'Error updating energy BRS status' }),
