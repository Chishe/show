...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        return new NextResponse(
            JSON.stringify({
                message: 'Quality HVAC status updated successfully',
                result: result.rows[0], 
            }),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
    } catch (error) {
        console.error('Error updating quality HVAC status:', error);
        return new NextResponse(
            JSON.stringify({ message: 'Error updating quality HVAC status' }),
