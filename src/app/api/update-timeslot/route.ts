...
export async function POST(req: NextRequest) {
  try {
    ...
    const selectResult = await pool.query(
      "SELECT jsonb_object_agg(ts.timeSlot, jsonb_build_object('target', ts.target, 'actual', ts.actual)) AS timeslots FROM timeSlots ts WHERE row_id = $1 GROUP BY row_id",
      [seq]
    );

    if (selectResult.rows.length === 0) {
      return NextResponse.json({ success: false, message: "Row not found" }, { status: 404 });
    }
    ...
    await pool.query(
      `UPDATE timeSlots SET ${type} = $1 WHERE row_id = $2 AND timeSlot = $3`,
