  ...
  try {
    ...
    try {
      ...
      const oldPartRes = await client.query(
        `SELECT DISTINCT partnumber FROM plan_${table} WHERE planDate = $1 AND jude = 'night'`,
        [date]
      );
      ...
      // DELETE old plans
      await client.query(
        `DELETE FROM plan_${table} WHERE planDate = $1 AND jude = 'night'`,
        [date]
      );
      ...
      for (const partnumber of removedParts) {
        ...
        if ((rowRes.rowCount ?? 0) > 0)
          ...
          await client.query(`DELETE FROM timeSlots_${table} WHERE row_id = $1 AND date = $2 AND jude = 'night'`, [rowId, date]);
        }
        await client.query(`DELETE FROM rows_${table} WHERE partNumber = $1 AND date = $2 AND jude = 'night'`, [partnumber, date]);
      }
      ...
      for (const [partnumber, totalQty] of qtySumMap.entries()) {
        ...
        if (checkRes.rowCount === 0) {
          ...
          await client.query(
            `UPDATE rows_${table} SET qty = $1 WHERE partNumber = $2 AND date = $3 AND jude = 'night'`,
      ...
      for (const [partnumber, timeSlotData] of Object.entries(grouped)) {
        ...
        for (const [timeSlot, targets] of Object.entries(timeSlotData)) {
          const checkTimeSlotRes = await client.query(
            `SELECT actual FROM timeSlots_${table} WHERE row_id = $1 AND timeSlot = $2 AND date = $3 AND jude = 'night' LIMIT 1`,
            [rowId, timeSlot, date]
          );
          ...
          if (checkTimeSlotRes.rowCount === 0) {
            ...
            await client.query(
              `UPDATE timeSlots_${table} SET target = $1, actual = $2 WHERE row_id = $3 AND timeSlot = $4 AND date = $5 AND jude = 'night'`,
