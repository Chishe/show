...
interface DeliveryData {
  ...
  shipping_status: "On Time" | "Delayed";
...
// 3. Main Component with typed props
export default function DeliveryStatusCard({ type }: DeliveryStatusCardProps) {
  const [open, setOpen] = useState<boolean>(false);
  const [data, setData] = useState<DeliveryData | null>(null);
  const [status, setStatus] = useState<DeliveryData["status"]>("Normal");
  const [shippingStatus, setShippingStatus] = useState<DeliveryData["shipping_status"]>("On Time");
  ...
  const handleUpdate = () => {
    const updatedData: Partial<DeliveryData> = {
      status,
    ...
    fetch(`/api/${editApiEndpoint}/${data.id}`, {
      ...
      .then((res) => res.json())
      .then((resData) => {
        if (resData.message === "Delivery status updated successfully") {
          toast.success("Delivery status updated successfully!");
          setOpen(false);
        } else {
          toast.error(resData.message || "Failed to update delivery status.");
        }
      })
      .catch((err) => {
        ...
        <div className="bg-[#465E86] p-2 rounded-b-lg">
          <p
            className={`flex justify-between ${
              data.shipping_status !== "On Time" ? "text-yellow-500" : ""
            }`}
          >
          ...
          <p
            className={`flex justify-between ${
              data.safety_stock > data.target_ll_patrol ? "text-yellow-500" : ""
            }`}
          >
        ...
        <DialogContent className="bg-[#182039] rounded-sm border-none">
          ...
          <div className="flex flex-col gap-4">
            ...
            <select
              ...
              value={status}
              onChange={(e) => setStatus(e.target.value as DeliveryData["status"])}
            >
            ...
            <label htmlFor="ShippingStatus" className="text-white">
              <span className="text-red-500">*</span> Shipping Status
            </label>
            <select
              id="ShippingStatus"
              className="bg-white p-1 rounded-sm"
              value={shippingStatus}
              onChange={(e) => setShippingStatus(e.target.value as DeliveryData["shipping_status"])}
            >
              <option value="On Time">On Time</option>
              <option value="Delayed">Delayed</option>
            </select>

            <label htmlFor="SafetyStock" className="text-white">
              Safety Stock
            </label>
            <input
              id="SafetyStock"
              ...
              onChange={(e) =>
                setData({ ...data, safety_stock: parseInt(e.target.value) || 0 })
              }
            />
          </div>
          ...
          <div className="flex flex-row gap-4">
            <button
              ...
              className="mt-4 w-full text-[#1890FF] py-2 rounded"
            >
              Cancel
            </button>
            <button
              onClick={handleUpdate}
              className="mt-4 w-full bg-[#1890FF] hover:bg-[#189FFF] text-white py-2 rounded"
            >
              Submit
            </button>
          </div>
